...
hash_nestlings<- new.env(hash = TRUE, parent=emptyenv(), size=length(banddata$Band.Number))
            ...
            if(!is.na( banddata$JDate[chicklist[i]] )){
              ...
              if(!is.na(nestdata$Fledge.Fail.date[b])){
                ...
                if((nestdata$Hatch.Date[b] < banddata$JDate[chicklist[i]]) &  
                  ...
                  daysold<-banddata$JDate[chicklist[i]]-nestdata$Hatch.Date[b]
                  ...
                  for(m in 1: length(chickmetrics)){
                    ...
                    nestdata[b, nestcol]<-banddata[chicklist[i], bandcol ]
                  }
                }
              }
            } 
