...
for (j  in 1:length(listfilenames)){
  ...
  nestdata$F.Day.measured[which(grepl("/", nestdata$F.Day.measured))] <- as.character(as.Date(nestdata$F.Day.measured[which(grepl("/", nestdata$F.Day.measured))] , format("%m/%d/%Y"))) 
  nestdata$M.Day.measured[which(grepl("/", nestdata$M.Day.measured))] <- as.character(as.Date(nestdata$M.Day.measured[which(grepl("/", nestdata$M.Day.measured))] , format("%m/%d/%Y"))) 
