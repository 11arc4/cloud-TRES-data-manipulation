...
ls(hash_malaria, all.names = TRUE) #Checks to see if the hash was made properly
...
nestdata_1988$F.Malaria.Status<-c()
j = 0
for(bandID in as.character(nestdata_1988$FemaleID)){
  ...
  if (exists(bandID, hash_malaria)){
    ...
    nestdata_1988$blooddate[j]=paste (malaria$Exact.date[malariaIdx],malaria$Year[malariaIdx])
