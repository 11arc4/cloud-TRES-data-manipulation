...
for (nestname in ls(globalData$nests, all.names = T)){
  ...
  #now we need to go through and check to see whether nests are renest are actually renests or not
  #If we don't know anyting about either of the birds involved, best to assume it's a first nest
  if(is.na(nest$femaleID$m_key) & is.na(nest$maleID$m_key)){
    ...
    if(is.na(nest$maleID$m_key)){
      ...
      if(fyear$nest$length==1){
        ...
        #If she has multiple nests then we need to find out which one of the nests we are looking at
        i=0
