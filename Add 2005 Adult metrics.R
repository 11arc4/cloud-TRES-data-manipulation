nestdatadir<-"~/Masters Thesis Project/Tree Swallow Data/Amelia TRES data 1975-2016/FINAL QC DONE"
...
filename<-"Nest Level Data 2005.csv"
    ...
    if(exists(band, F.hash_2005bands)){
      ...
      if(is.na(nestdata$F.Day.measured[bandIDx])){
        ...
        if(as.Date(nestdata$F.Day.measured[bandIDx], format="%Y-%m-%d")< v | 
           as.Date(nestdata$F.Day.measured[bandIDx], format="%Y-%m-%d" )< as.Date("4/30/2005", format="%m/%d/%Y") ){
          nestdata$F.Day.measured[bandIDx] <- as.character(v)
